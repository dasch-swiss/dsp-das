    ...
    ngOnInit() {
        ...
        if (this._route.snapshot) {
            ...
            if (this._route.snapshot.params.id) {
                this.ontologyIri = decodeURIComponent(
                ...
                this._dspApiConnection.admin.usersEndpoint
                    ...
                    .subscribe(
                        (userResponse: ApiResponseData<UserResponse>) => {
                            this.projectMember =
                                userResponse.body.user.projects.some(
                                    (p) =>
                                        p.shortcode === this.project.shortcode
                                );
                ...
                (response: OntologiesMetadata) => {
                    if (!response.ontologies.length) {
                        ...
                        if (response.ontologies.length === 1 && !this.beta) {
                            // open this ontology
                            this.openOntologyRoute(
                                response.ontologies[0].id,
                                this.view
                            );
                            this.ontologyIri = response.ontologies[0].id;
                            this.lastModificationDate =
                                response.ontologies[0].lastModificationDate;
                        }
