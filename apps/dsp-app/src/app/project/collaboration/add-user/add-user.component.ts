...
export class AddUserComponent implements OnInit {
  ...
  buildForm(): void {
    ...
    // get all users
    this._userApiService.list().subscribe(
      response => {
    ...
    this.filteredUsers = this.selectUserForm.controls[
      ...
      startWith(''),
      map((user) =>
        user.length >= 2 ? this.filter(this.users, user) : []
      )
    );
  ...
  addUser(val: string) {
    ...
    this._userApiService.get(val, 'username')
      .subscribe(
        response => {
          ...
          this.isAlreadyMember = !!response.user.projects.find(
          ...
          if (!this.isAlreadyMember) {
            ...
            this._userApiService.addToProjectMembership(
