...
describe('ListComponent', () => {
    ...
    describe('Displaying lists', () => {
        ...
        it('should open a list', async () => {
            ...
            expect(testHostComponent.listComponent.list.id).toEqual(
                'http://rdfh.ch/lists/0001/mockList01'
            );
        });
    });
    ...
    describe('Delete list with lists remaining after deletion', () => {
        ...
        it('should delete a root node and the first of the remaining lists should be shown', async () => {
            const deleteListResponse: DeleteListResponse =
            ...
            testHostFixture.whenStable().then(() => {
                ...
                ).toHaveBeenCalledWith('http://rdfh.ch/lists/0001/mockList01');
                expect(
                    listSpy.admin.listsEndpoint.deleteListNode
                ).toHaveBeenCalledTimes(1);

                expect(testHostComponent.listComponent.lists.length).toEqual(1);
                expect(testHostComponent.listComponent.lists[0].id).toEqual(
                    'http://rdfh.ch/lists/0001/mockList02'
                );

                expect(testHostComponent.listComponent.listIri).toEqual(
                    testHostComponent.listComponent.lists[0].id
                );
            });
        });
    });
