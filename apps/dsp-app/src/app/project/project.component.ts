...
export class ProjectComponent implements OnInit {
    ...
    open(route: string, id?: string) {
        ...
        if (route === 'add-ontology' || route === 'add-list') {
            if (!this.project || !this.project.status) {
                // project is deactivated. Do not reroute
                return;
            }
        }
        if (id) {
            if (route === 'ontology') {
                this._router.navigate(
                    [route, encodeURIComponent(id), 'editor', 'classes'],
                    { relativeTo: this._route }
                );
            } else {
                this._router.navigate([route, encodeURIComponent(id)], {
                    relativeTo: this._route,
                });
            }
        } else {
