 ...
 */
@Component({
    template: ` <app-search-select-resource-class
        #selectResClass
        [formGroup]="form"
        [resourceClassDefinitions]="resourceClassDefs"
        (resourceClassSelected)="resClassSelected($event)"
    ></app-search-select-resource-class>`,
})
class TestHostComponent implements OnInit {
    @ViewChild('selectResClass')
    selectResourceClass: SearchSelectResourceClassComponent;
    ...
    ngOnInit() {
        ...
        this.resourceClassDefs = MockOntology.mockReadOntology(
            'http://0.0.0.0:3333/ontology/0001/anything/v2'
        ).getClassDefinitionsByType(ResourceClassDefinition);
        this.resourceClassDefs = this._sortingService.keySortByAlphabetical(
            this.resourceClassDefs,
            'label'
        );
    }
...
describe('SearchSelectResourceClassComponent', () => {
    ...
    it('should initialise the resource class definitions correctly', () => {
        expect(
            testHostComponent.selectResourceClass.resourceClassDefinitions
                .length
        ).toEqual(12);
    });
    ...
    it('should emit the Iri of a selected resource class', async () => {
        ...
        expect(
            testHostComponent.selectResourceClass.selectedResourceClassIri
        ).toBe(false);
        ...
        expect(testHostComponent.selectedResClassIri).toEqual(
            'http://0.0.0.0:3333/ontology/0001/anything/v2#BlueThing'
        );

        expect(
            testHostComponent.selectResourceClass.selectedResourceClassIri
        ).toEqual('http://0.0.0.0:3333/ontology/0001/anything/v2#BlueThing');
    });
    ...
    it('should update the resource class definitions when the @Input changes', async () => {
        ...
        testHostComponent.selectResourceClass['_selectedResourceClassIri'] =
            'http://0.0.0.0:3333/ontology/0001/anything/v2#BlueThing';

        // get resource class defs
        testHostComponent.resourceClassDefs = MockOntology.mockReadOntology(
    ...
    it('should unsubscribe from from changes on destruction', () => {
        expect(
            testHostComponent.selectResourceClass.ontologyChangesSubscription
                .closed
        ).toBe(false);
