 ...
 */
@Component({
    template: ` <app-search-select-property
        #selectProp
        [formGroup]="form"
        [index]="0"
...
describe('SearchSelectPropertyComponent', () => {
    ...
    it('should initialise the Inputs correctly', () => {
        ...
        expect(
            testHostComponent.selectProperty.activeResourceClass
        ).toBeUndefined();
        expect(
            Object.keys(testHostComponent.selectProperty.properties).length
        ).toEqual(24);
    ...
    it('should set the active property', async () => {
        expect(
            testHostComponent.selectProperty.propertySelected
        ).toBeUndefined();
        ...
        expect(testHostComponent.selectProperty.propertySelected.id).toEqual(
            'http://0.0.0.0:3333/ontology/0001/anything/v2#hasBlueThing'
        );

        expect(
            testHostComponent.selectProperty.specifyPropertyValue.property
        ).toBeDefined();
        expect(
            testHostComponent.selectProperty.specifyPropertyValue.property.id
        ).toEqual('http://0.0.0.0:3333/ontology/0001/anything/v2#hasBlueThing');
    });
    ...
    it('should get the specified value for the selected property', async () => {
        ...
        const propWithVal =
            testHostComponent.selectProperty.getPropertySelectedWithValue();

        expect(propWithVal.property.id).toEqual(
            'http://0.0.0.0:3333/ontology/0001/anything/v2#hasInteger'
        );

        expect(propWithVal.valueLiteral.value).toEqual(
            new ValueLiteral('1', Constants.XsdInteger)
        );
        expect(propWithVal.valueLiteral.comparisonOperator).toEqual(
            new Equals()
        );
    ...
    it('should reinitialise the properties', async () => {
        ...
        testHostComponent.propertyDefs = MockOntology.mockReadOntology(
            'http://0.0.0.0:3333/ontology/0001/anything/v2'
        ).getPropertyDefinitionsByType(ResourcePropertyDefinition);
    ...
    it('should remove the control from the parent form and unsubscribe from value changes when destroyed', waitForAsync(() => {
        expect(
            testHostComponent.selectProperty.propertyChangesSubscription.closed
        ).toBe(false);
        ...
        testHostFixture.whenStable().then(() => {
            ...
            expect(
                testHostComponent.selectProperty.propertyChangesSubscription
                    .closed
            ).toBe(true);
        });
    }));
