    ...
    implements OnChanges, OnDestroy, AfterViewInit
{
    ...
    renderRegions(): void {
        ...
        for (const image of this.images) {
            ...
            for (const geom of geometries) {
                ...
                const commentValue = geom.region.properties[
                    Constants.HasComment
                ]
    ...
    openReplaceFileDialog() {
        const propId =
            this.parentResource.properties[Constants.HasStillImageFileValue][0]
                .id;
    ...
    private _uploadRegion(
        ...
        const x1 =
            Math.max(Math.min(startPoint.x, imageSize.x), 0) / imageSize.x;
        const x2 = Math.max(Math.min(endPoint.x, imageSize.x), 0) / imageSize.x;
        const y1 =
            Math.max(Math.min(startPoint.y, imageSize.y), 0) / imageSize.y;
    ...
    private _addRegionDrawer() {
        new OpenSeadragon.MouseTracker({
            ...
            releaseHandler: () => {
                if (this.regionDrawMode) {
                    const imageSize = this._viewer.world
                        .getItemAt(0)
                        .getContentSize();
                    const startPoint =
                        this._viewer.viewport.viewportToImageCoordinates(
