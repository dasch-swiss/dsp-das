...
@Component({
    template: ` <app-link-value
        ...
        (referredResourceClicked)="refResClicked($event)"
        (referredResourceHovered)="refResHovered($event)"
    ></app-link-value>`,
})
class TestHostDisplayValueComponent implements OnInit {
    ...
    ngOnInit() {
        MockResource.getTestThing().subscribe((res) => {
            const inputVal: ReadLinkValue = res.getValuesAs(
    ...
    describe('display and edit a link value', () => {
        ...
        beforeEach(() => {
            ...
            valueComponentDe = hostCompDe.query(
                By.directive(LinkValueComponent)
            );
        ...
        it('should display an existing value', fakeAsync(() => {
            expect(
                testHostComponent.inputValueComponent.displayValue
                    .linkedResourceIri
            ...
            expect(
                testHostComponent.inputValueComponent.valueFormControl
                    .value instanceof ReadResource
            ).toBe(true);
            expect(
        ...
        it('should search for resources by their label', fakeAsync(() => {
            ...
            expect(
                testHostComponent.inputValueComponent.resources.length
            ).toEqual(1);
            expect(
                testHostComponent.inputValueComponent.resources[0].id
            ).toEqual('http://rdfh.ch/0001/IwMDbs0KQsaxSRUTl2cAIQ');
        }));
