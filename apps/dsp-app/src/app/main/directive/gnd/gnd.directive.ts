    ...
    ngOnChanges() {
        if (this._gnd.length < 30) {
            if (this._gnd.indexOf(GNDConstants.GND_PREFIX) === 0) {
                // gnd/iaf identifier
                this._ele.nativeElement.innerHTML = `<a href="${
                    GNDConstants.GND_RESOLVER +
                    this._gnd.replace(GNDConstants.GND_PREFIX, '')
                }" target="_blank">${this._gnd}</a>`;
            } else if (this._gnd.indexOf(GNDConstants.VIAF_PREFIX) === 0) {
                // viaf identifier
                this._ele.nativeElement.innerHTML = `<a href="${
                    GNDConstants.VIAF_RESOLVER +
                    this._gnd.replace(GNDConstants.VIAF_PREFIX, '')
                }" target="_blank">${this._gnd}</a>`;
            } else {
