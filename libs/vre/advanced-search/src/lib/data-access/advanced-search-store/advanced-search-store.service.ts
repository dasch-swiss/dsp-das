...
export class AdvancedSearchStoreService extends ComponentStore<AdvancedSearchState> {
  ...
  searchButtonDisabled$: Observable<boolean> = this.select(
    ...
    (ontology, resourceClass, propertyFormList) => {
      ...
      const areSomePropertyFormItemsInvalid = propertyFormList.some(prop =>
        this.isPropertyFormItemListInvalid(prop)
      );
  ...
  addChildPropertyFormList(property: PropertyFormItem): void {
    ...
    const currentSearchValue = currentPropertyFormList[indexInPropertyFormList]
        ...
        switchMap(count => {
          ...
          if (count > 0) {
            return this._advancedSearchService
              .getResourcesList(searchItem.value, searchItem.objectType)
              .pipe(take(1));
          } else {
  ...
  operatorsMap(): Map<string, string[]> {
    return new Map([
      ...
      [
        ...
        [Constants.IntValue, Constants.DecimalValue, Constants.DateValue],
      ],
      [
        Operators.LessThan,
        [Constants.IntValue, Constants.DecimalValue, Constants.DateValue],
