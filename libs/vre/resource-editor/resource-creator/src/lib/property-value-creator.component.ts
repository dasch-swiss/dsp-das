    ...
    @if (template) {
      <div style="display: flex; position: relative" [ngClass]="{ works: isValid$ | async }">
        ...
        <div class="action-buttons">
          @if (canRemoveValue) {
            <button mat-icon-button type="button" color="primary" [matTooltip]="'resourceEditor.resourceCreator.propertyValueCreator.remove' | translate" (click)="removeValue.emit()">
          ...
          <button
            ...
            [matTooltip]="commentIsNotNull ? ('resourceEditor.resourceCreator.propertyValueCreator.removeComment' | translate) : ('resourceEditor.resourceCreator.propertyValueCreator.addComment' | translate)">
