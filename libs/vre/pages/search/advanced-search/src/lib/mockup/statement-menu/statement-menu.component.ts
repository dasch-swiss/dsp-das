...
@Component({
  ...
  template: `
    <mat-menu #menu="matMenu" class="statement-menu" [hasBackdrop]="true">
      <!-- Existing statements section - non-menu-item content that should not close menu -->
      @if (statements.length > 0) {
        ...
        <div class="statements-container" (click)="$event.stopPropagation()">
  ...
  styles: [
    `
      ...
      ::ng-deep .property-menu-item,
      ::ng-deep .operator-menu-item {
        font-size: 14px;
      }

      ::ng-deep .property-menu-item mat-icon,
      ::ng-deep .operator-menu-item mat-icon {
        font-size: 18px;
        width: 18px;
        height: 18px;
        opacity: 0.7;
...
export class StatementMenuComponent {
  private readonly cdr = inject(ChangeDetectorRef);

  @Input() statements: FilterChipData[] = [];
  @Input() properties: PropertyOption[] = [];
  @Input({ required: true }) resourceClass!: IriLabelPair;
  ...
  get filteredProperties(): PropertyOption[] {
    ...
    return this.properties.filter(p => p.label.toLowerCase().includes(filter));
  }
  ...
  onAddStatement(prop: PropertyOption, operator: string, value: string): void {
    const statement: Partial<FilterChipData> = {
      resourceClassIri: this.resourceClass.iri,
      resourceClassLabel: this.resourceClass.label,
  ...
  onEditApply(stmt: FilterChipData): void {
    ...
    this.statementApply.emit(updatedStatement);
