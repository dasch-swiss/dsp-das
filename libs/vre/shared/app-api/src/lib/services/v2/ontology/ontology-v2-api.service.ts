...
} from './resource-property-definition-with-all-languages.interface';
...
export class OntologyV2ApiService extends BaseApi {
    ...
    deleteResourceClass(iri: string, lastModificationDate: string) {
        return this._http.delete(`${this.baseUri}/classes/${encodeURIComponent(iri)}?lastModificationDate=${lastModificationDate}`);
    }

    deleteResourceClassComment(iri: string, lastModificationDate: string) {
        return this._http.delete<ResourceClassDefinitionWithAllLanguages>(`${this.baseUri}/classes/comment/${encodeURIComponent(iri)}?lastModificationDate=${lastModificationDate}`);
    }
    ...
    deleteResourceProperty(iri: string, lastModificationDate: string) {
        return this._http.delete(`${this.baseUri}/properties/${encodeURIComponent(iri)}?lastModificationDate=${lastModificationDate}`);
    }

    deleteResourcePropertyComment(iri: string, lastModificationDate: string) {
        return this._http.delete(`${this.baseUri}/properties/comment/${encodeURIComponent(iri)}?lastModificationDate=${lastModificationDate}`);
    }
    ...
    canReplaceCardinalityOfResourceClassWith(resourceClassIri: string, propertyIri: string, newCardinality: string) {
        return this._http.get(`${this.baseUri}/canreplacecardinalities/${encodeURIComponent(resourceClassIri)}?propertyIri=${encodeURIComponent(propertyIri)}&newCardinality=${newCardinality}`);
    }
