...
export class ListApiService extends BaseApi {
    ...
    get(iri: string) {
        return this._http.get<ListResponse | ListChildNodeResponse>(
            this._listRoute(iri)
        );
    }
    ...
    update(iri: string, updatedGroup: UpdateGroupRequest) {
        return this._http.put<ListsResponse>(
            this._listRoute(iri),
            updatedGroup
        );
    }
    ...
    delete(iri: string) {
        return this._http.delete<DeleteListResponse | DeleteListNodeResponse>(
            this._listRoute(iri)
        );
    }

    listInProject(projectIri: string) {
        return this._http.get<ListsResponse>(
            `${this.baseUri}?projectIri=${encodeURIComponent(projectIri)}`
        );
    }

    getInfo(iri: string) {
        return this._http.get<ListInfoResponse>(
            `${this.baseUri}/infos/${encodeURIComponent(iri)}`
        );
    }

    getNodeInfo(iri: string) {
        return this._http.get<ListNodeInfoResponse>(
            `${this._listRoute(iri)}/info}`
        );
    }

    updateInfo(iri: string, updatedList: UpdateListInfoRequest) {
        return this._http.put<ListInfoResponse>(
            this._listRoute(iri),
            updatedList
        );
    }
    ...
    updateChildName(
        iri: string,
        updatedChildNodeName: UpdateChildNodeNameRequest
    ) {
        return this._http.put<ChildNodeInfoResponse>(
            `${this._listRoute(iri)}/name`,
            updatedChildNodeName
        );
    }

    updateChildLabels(iri: string, labels: UpdateChildNodeLabelsRequest) {
        return this._http.put<ChildNodeInfoResponse>(
            `${this._listRoute(iri)}/labels`,
            labels
        );
    }
    ...
    deleteChildComments(iri: string) {
        ...
        return this._http.delete<DeleteChildNodeCommentsResponse>(
            `${this.baseUri}/comments/${encodeURIComponent(iri)}`
        );
    }

    createChildNode(parentNodeIri: string, node: CreateChildNodeRequest) {
        return this._http.post<ListNodeInfoResponse>(
            this._listRoute(parentNodeIri),
            node
        );
    }

    updateChildNodePosition(
        iri: string,
        repositionRequest: RepositionChildNodeRequest
    ) {
        return this._http.put<RepositionChildNodeResponse>(
            `${this._listRoute(iri)}/position`,
            repositionRequest
        );
    }
    ...
    updateChildNode(iri: string, updatedNode: UpdateChildNodeRequest) {
        ...
        if (
            updatedNode.name === undefined &&
            updatedNode.labels === undefined &&
            updatedNode.comments === undefined
        ) {
            throw new Error(
                'At least one property is expected from the following properties: name, labels, comments.'
            );
        }
        return this._http.put<ChildNodeInfoResponse>(
            this._listRoute(iri),
