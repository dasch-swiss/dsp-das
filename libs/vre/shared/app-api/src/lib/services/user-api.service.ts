...
export class UserApiService extends BaseApi {
    ...
    updateStatus(iri: string, status: boolean) {
        return this._http.put<UserResponse>(`${this._userRoute(iri)}/Status`, { status });
    }
    ...
    updateSystemAdminMembership(iri: string, isSystemAdmin: boolean) {
        return this._http.put<UserResponse>(`${this._userRoute(iri)}/SystemAdmin`, { systemAdmin: isSystemAdmin });
    }
