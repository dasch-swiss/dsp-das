...
export class UserState {
  ...
  loadUser(
    ...
    return this._userApiService.get(username, 'username')
      .pipe(
        take(1),
        map(response => {
          ctx.setState({ ...ctx.getState(), isLoading: false, user: response.user });
  ...
  loadUserContentByIriAction(
    ...
    ctx.patchState({ isLoading: true });
    return this._userApiService.get(iri)
      .pipe(
        take(1),
        tap({
          next: response => {
  ...
  loadUsersAction(
    ...
    ctx.patchState({ isLoading: true });
    return this._userApiService.list()
      .pipe(
        take(1),
        tap({
          next: response => {
  ...
  createUserAction(
    ...
    ctx.patchState({ isLoading: true });
    return this._userApiService.create(userData)
      .pipe(
        take(1),
        tap({
          next: response => {
