...
@Component({
    ...
    template: `
      <app-add-value *ngIf="resourcePropertyDefinition" #testAddVal [resourcePropertyDefinition]="resourcePropertyDefinition"
      [parentResource]="readResource"></app-add-value>`
})
class DspAddValueTestComponent implements OnInit {
    ...
    assignResourcePropDef(propIri: string) {
        const definitionForProp = this.readResource.entityInfo.getAllPropertyDefinitions()
            .filter( (resourcePropDef: ResourcePropertyDefinition) => resourcePropDef.id === propIri) as ResourcePropertyDefinition[];
